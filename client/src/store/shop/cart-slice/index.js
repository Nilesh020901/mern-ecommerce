...
export const addToCart = createAsyncThunk(
    ...
    async ({ userId, productId, quantity }, { dispatch }) => {
        await axios.post(
            `http://localhost:5000/api/shop/cart/add-to-cart`,
...
export const deleteFromCart = createAsyncThunk(
    ...
    async ({ userId, productId }) => {
        const response = await axios.delete(`http://localhost:5000/api/shop/cart/delete-cart-items/${userId}/${productId}`);
        return response?.data;
    }
)
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateCartItems.pending, (state) => {
                ...
                console.log("deleteFromCart called with:", action.payload);
                state.isLoading = false;
                state.cartItems = action.payload.data;
